var handle = exports, event // placeholder that will be set to app.event on init
, db = {
	descriptors : {}
}, utils = require('./utils.js'), utils_gs = require('./utils-gs.js'), config = require('./config.js'), serialReadBuffer = [];
;
...
handle.init = function(app) {
	...
	console.log();
	utils.logText('Ground Station starting in ' + utils.colors.warn
			+ config.env + utils.colors.reset + ' mode', 'INF');
}
				...
				.emit(
						...
						function(new_descriptors) {
							if (new_descriptors.length) {
								utils.logText('Descriptor ' + desc_typeid
										+ ' retrieved from CC');
								...
								utils
										.logText('RAP dropped - descriptor not available from CC');
						});
	}
}
...
handle.TAP = function(rapbytes, callback) {
	var rap = {};
	if (rapbytes.length < 11)
		return; // This should protect all indexing below
	...
	if (!utils_gs.verifyChecksums(rap, rapbytes[rapbytes.length - 2],
			rapbytes[rapbytes.length - 1])) {
		utils.logText('TAP dropped - wrong RAP checksum', 'INF',
				utils.colors.warn);
		return;
	}
...
handle.doRAPtoTAP = function(rap, tapbytes, tap_desc, callback) {
	...
	for ( var i in tap_desc.h) {
		if (tap_desc.h[i].f) { // for each item in the tap header
			var bytesOfNumber = tapbytes.slice(bytecount,
					bytecount += tap_desc.h[i].l);
	...
	for ( var i in tap_desc.p) {
		if (tap_desc.p[i].f) { // for each item in the tap repeatable elements
			if (tap_desc.p[i].l < 0) { // variable length data
				...
				result = tapbytes
						.slice(bytecount, bytecount += tap_desc.p[i].l);
			}
	...
	for ( var i in cap_desc.h) {
		if (cap_desc.h[i].f) { // for each item in the cap header
			if (!cap.h[cap_desc.h[i].f] && (cap_desc.h[i].f != 'l')) { // if a field is missing, and that field is not the cap length
				utils.logText('CAP dropped - missing field ' + cap_desc.h[i].f,
						'INF', utils.colors.warn);
				return;
			}
			if (cap_desc.h[i].f == 'l')
				continue;
	...
	for ( var i in cap_desc.p) {
		if (cap_desc.p[i].f) { // for each item in the cap header
			if (!cap.p[cap_desc.p[i].f]) { // if a field is missing
				utils.logText('CAP dropped - missing field ' + cap_desc.p[i].f,
						'INF', utils.colors.warn);
				return;
			}
...
handle.SerialRead = function(newdata) { // newdata can be either Buffer or Array. The extendArray function can handle either.
	...
	if (rapbytes.length) {
		...
		mon.collection('TAPS').insert(rawTAP);
	}
}
...
handle.SerialWrite = function(data) {
	port.write(new Buffer(data), function(err, results) {
		if (err)
			utils.logText('Serial write error - ' + err, 'ERR');
