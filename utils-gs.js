module.exports = function() {
	...
	// Extend the given array with the provided newdata, which can be either
	// Buffer or Array
	utils.extendArray = function(arrcurr, datanew) {
		for ( var i = 0; i < datanew.length; i++)
			arrcurr.push(datanew[i]);
	}
	...
	utils.augmentChecksums = function() {
		if (arguments.length < 2)
			return;
		var obj = arguments[0];
		if (obj.checksumA === undefined)
			obj.checksumA = 0;
		if (obj.checksumB === undefined)
			obj.checksumB = 0;
	...
	// compare the checksums of the passed object at arg[0] with the checksums
	// passed at arg[1,2]
	...
	utils.bytesToNumber = function() {
		...
		if (arguments.length == 1)
			array = arguments[0];
		for ( var k in array) {
			value = ((value << 8) >>> 0) + (array[k] >>> 0); // careful with
			// << because
			// large numbers
			// roll over
		}
	...
	// convert the input number, string, or array to a series of bytes,
	// optionally padded to a specified length
	utils.toBytes = function(arrayIn, input, length) {
		...
		if (typeof input == 'number') {
			while (true) {
				...
				if (input < 256)
					break;
				input >>>= 8; // triple shift ensures non-negative result due
				// to JS number size
			}
