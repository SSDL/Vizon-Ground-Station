module.exports = function(app) {
	var event = app.event, io = require('socket.io-client'), crypto = require('crypto'), utils = require('./utils.js'), config = require('./config.js'), handle = require('./handlers.js'), socket = {};

	// begin socket setup
	var socketRetry = setInterval(function() { // this is the only way to loop
		// if not connected
	...
	// other possible standard socket events include 'connecting',
	// 'reconnecting', 'reconnect'
	.on('connect_failed', function() {
		utils.logText('Connection to CC failed', 'ERR');
	}).on('reconnect_failed', function() {
		utils.logText('Reconnection to CC failed', 'ERR');
	}).on('error', function(err) {
		utils.logText('Socket ' + (config.dev && err ? err : ''), 'ERR');
	})
	...
	.on(
			...
			function(challenge, callback) {
				var response = crypto.createHmac(challenge.alg, config.key)
						.update(challenge.data).digest(challenge.enc);
				callback(response);
	...
	.on('relay', function(data) {
		...
		handle.CAP(packet); // add callback to ensure bytes written? or let TAP2
		// (cmd echo) handle that
	})
