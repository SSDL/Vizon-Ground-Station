module.exports = function(app) {
	var event = app.event, handle = require('./handlers.js'), utils = require('./utils.js'), endpoints = {};
	...
	handle.PortWrite = function(data) {
		endpoints.port.write(data, function(err, results) {
			if (err)
				utils.log('Serial write error: ' + err);
			if (results != 0)
				utils.logText('Serial data write - ' + results + ' bytes');
		});
	}
